  ...
  if (Array.isArray(obj)) {
    ...
    for (name in obj) {
      //don't process attributes as element
        ...
        var child = obj[name];
        if (child[self.options.attributesKey] && child[self.options.attributesKey].xsi_type)
        {
            var xsiType = child[self.options.attributesKey].xsi_type;
...
function open_wsdl(uri, options, callback) {
  ...
  if (!/^http/.test(uri)) {
    ...
    }, request_headers, request_options).setTimeout(2000, function ()
        {
            callback(new Error('Timeout getting WSDL from URL: ' + uri));
        });
  }
